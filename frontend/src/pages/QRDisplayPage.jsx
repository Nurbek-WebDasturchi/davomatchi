...
export default function QRDisplayPage() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${qr.groupName}-QR.png`, { type: 'image/png' });
      await navigator.share({ title: `${qr.groupName} QR Kodi`, files: [file] });
    } catch {}
  };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingBottom: '80px' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '11px' }}>{qr?.groupName}</p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          background: 'var(--bg-card)', border: '1px solid var(--border)',
          borderRadius: 'var(--radius-lg)', padding: '14px', width: '100%',
        }}>
          <p style={{ color: 'var(--text-muted)', fontSize: '10px', fontWeight: 700, marginBottom: '6px' }}>
            QR TOKEN
          </p>
          <p style={{
            fontFamily: 'var(--font-mono)', fontSize: '11px',
            color: 'var(--accent-blue-light)', wordBreak: 'break-all',
          }}>
            {qr?.qrToken}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', width: '100%' }}>
          <button onClick={handleDownload} style={{
            ...
            borderRadius: 'var(--radius-lg)', fontSize: '13px', fontWeight: 800,
          }}>
            ⬇️ Yuklab olish
          </button>
        ...
        <div style={{
          ...
          <p style={{ color: 'var(--accent-blue-light)', fontSize: '12px', fontWeight: 800, marginBottom: '10px' }}>
            ℹ️ Qanday ishlaydi?
          </p>
          {[
            ...
            <p key={t} style={{ color: 'var(--text-secondary)', fontSize: '12px', marginBottom: '5px' }}>{t}</p>
          ))}
        </div>
      </div>
