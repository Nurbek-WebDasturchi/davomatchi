    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingBottom: '80px' }}>
      ...
      <div style={{
        ...
        <button onClick={() => navigate(-1)} style={{
          ...
          borderRadius: '8px', padding: '8px 10px', color: 'var(--text-primary)', fontSize: '16px',
        }}>←</button>
        ...
        {!isCourse && user.role === 'admin' && (
          <button onClick={() => navigate(`/qr/${id}`)} style={{
            background: 'rgba(168,85,247,0.12)', border: '1px solid rgba(168,85,247,0.3)',
            borderRadius: '8px', padding: '7px 12px', color: 'var(--accent-purple)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
              {[
                ...
                { label: 'Kelmadi', val: (data?.totalStudents||0) - (data?.presentCount||0),         color: 'var(--accent-red)'   },
              ].map(({ label, val, color }) => (
                <div key={label} style={{
                  background: 'var(--bg-card)', border: '1px solid var(--border)',
                  ...
                  <p style={{ color: 'var(--text-muted)', fontSize: '10px', fontWeight: 700, marginTop: '2px' }}>{label}</p>
                </div>
              ))}
            </div>
                ...
                {filtered?.map((s, i) => (
                  <div key={s.id} style={{
                    ...
                    <div style={{
                      ...
                      color: s.is_present ? 'var(--accent-green)' : 'var(--text-muted)',
                      flexShrink: 0,
                    }}>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      {s.is_present ? (
                        <>
                          <p style={{ color: 'var(--accent-green)', fontSize: '10px', fontWeight: 700 }}>✅ Keldi</p>
                          {s.scanned_at && (
                            <p style={{ color: 'var(--text-muted)', fontSize: '10px' }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', fontSize: '10px', fontWeight: 700 }}>❌ Kelmadi</p>
                      )}
                    </div>
                  </div>
                ))}
