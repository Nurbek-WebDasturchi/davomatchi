...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      api.get('/attendance/today').then(res => setData(res.data)).catch(() => {});
    }, 30000);
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)' }}>
      <div style={{ width: 40, height: 40, border: '3px solid var(--border)', borderTop: '3px solid var(--accent-blue)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
    </div>
  );

  const { totals, byCourse } = data || {};
  const pct = totals?.total_students > 0
    ? Math.round(totals.present_today / totals.total_students * 100) : 0;
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingBottom: '80px' }}>
      <div style={{
        ...
        <div>
          <h1 style={{ fontSize: '18px', fontWeight: 900 }}>
            {user?.role === 'director' ? '👨‍💼 Direktor' : '👨‍💼 Direktor o\'rinbosari'}
          </h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '11px' }}>{today}</p>
        </div>
      ...
      <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div style={{
          ...
          <p style={{ fontSize: '11px', color: 'var(--accent-blue-light)', fontWeight: 700, marginBottom: '2px' }}>Xush kelibsiz 👋</p>
          <p style={{ fontSize: '18px', fontWeight: 900 }}>{user?.firstName} {user?.lastName}</p>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text-secondary)' }}>Bugungi davomat</span>
            <span style={{
        ...
        {byCourse?.map(course => {
          const p = course.total_students > 0
            ? Math.round(course.present_count / course.total_students * 100) : 0;
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                ...
                <span style={{ fontSize: '16px', fontWeight: 900, color, background: `${color}15`, padding: '4px 10px', borderRadius: '8px' }}>
                  {p}%
                </span>
              </div>
